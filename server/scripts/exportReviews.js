...
async function exportReviews() {
  try {
    ...
    const reviews = await findAllRecords(Review);
    console.log(`${reviews.length} Review records found`);

    const outputFile = path.join(__dirname, 'reviews.json');
    fs.writeFileSync(outputFile, JSON.stringify(reviews, null, 2));
    console.log(`Review records exported successfully to ${outputFile}`);

  } catch (error) {
    console.error('Error exporting Review records:', error);
  } finally {
