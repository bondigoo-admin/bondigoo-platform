...
const MasterActionPanel = ({ user, onUserUpdate, onSetFeeOverrideClick }) => { // Receive onSetFeeOverrideClick prop
  ...
  // State management for modals is removed, except for the AlertDialogs' content
  const [suspensionReason, setSuspensionReason] = React.useState('');
  const [impersonateReason, setImpersonateReason] = React.useState('');
  const [passwordResetReason, setPasswordResetReason] = React.useState('');
  const [verifyEmailReason, setVerifyEmailReason] = React.useState('');
  ...
  return (
    // The <> and the modal rendering at the bottom are removed
    <div className="flex flex-wrap gap-2 rounded-lg bg-muted/50 p-4 dark:bg-zinc-800/50">
        <h3 className="mb-2 w-full text-md font-semibold">{t('userManagement.godMode.title', 'Master Mode Actions')}</h3>
      
        {/* All AlertDialog sections for Suspend, Change Role, etc. remain unchanged */}
        {/* ... (Suspend/Reactivate, Change Role, Reset Password, etc.) ... */}
        ...
        {/* --- UPDATED BUTTON --- */}
        {user.role === 'coach' && (
          <Button variant="outline" onClick={onSetFeeOverrideClick}>
            {t('userManagement.actions.setFeeOverride', 'Set Fee Override')}
          </Button>
        )}
    </div>
  );
