...
import { getProgramLearningOutcomes } from '../../services/programAPI';
...
const LearningOutcomeFilter = ({
  ...
  const fetchAndSetResults = useCallback(async (term) => {
    setIsLoading(true);
    try {
      const results = await getProgramLearningOutcomes(term, i18n.language);
    ...
    <div className="relative w-full" ref={dropdownRef}>
      <div className="relative">
        <Input
          ...
          placeholder={placeholder || t('search_learning_outcomes')}
          ...
          aria-label={placeholder || t('search_learning_outcomes')}
        />
        <button
          type="button"
...
export default memo(LearningOutcomeFilter, (prev, next) => isEqual(prev.selectedItems, next.selectedItems));